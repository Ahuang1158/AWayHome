...
mclvl4 = Animation("scientist.png",3,game,196/4,176/2)
mclvl4.resizeBy(50)
...
while not game.over:
    ...
    for index in range(50):
        ...
        cat[index].moveTowards(mclvl4,2)
        fish.moveTo(x , y)        
        ...
        if cat[index].collidedWith(fish):
            ...
            ct.play()
            cat[index].visible = False
        if cat[index].collidedWith(mclvl4):
            mclvl4.health -= 10
            ct.play()
            cat[index].visible = False
        if mclvl4.health <= 3:
