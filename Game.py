...
bklv3 = Image("beach.jpg",game)
game.setBackground(bklv3)
bklv3.resizeTo(900,800)
...
while not game.over:
    ...
    for index in range(50):
        x = randint (300,600)
        y = randint(450,1300)
        cat3[index].moveTowards(fish,5)
        cat3[index].moveTowards(mclvl4,2)
        fish.moveTo(x , y)        
        ...
        if keys.Pressed[K_SPACE]:
            ...
            fish.resizeTo(70,70)
            fish.visible = True
        if cat3[index].collidedWith(fish):
            fish.visible = False
            ct.play()
        ...
        if mclvl4.health <= 3:
            Game.over = True
        if cat3[index].isOffScreen("bottom") and cat3[index].visible:
            catPassed2 += 1 
            cat3[index].visible = False
        if catPassed2 >= 10:
            key.draw()
            key.moveTo(550,450)
        if key.collidedWith(mclvl4):
            key.draw()
            key.moveTo(550,450)
            game.over = False
    ...
    game.drawText("Press Space To Release Fish " + str(),300, 100)
